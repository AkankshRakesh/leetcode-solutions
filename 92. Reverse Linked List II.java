...
class Solution {
    ...
    public void reverse(ListNode head, ListNode tail, int currPos, int right, ListNode revHead){
        ListNode next = new ListNode(-1000);
        if(tail.next != null) next = tail.next;

        tail.next = head;
        if(currPos >= right - 1){
            if(next.val == -1000) revHead.next = null;
    ...
    public ListNode reverseBetween(ListNode head, int left, int right) {
        ...
        System.out.println(nodeAtRight.val);
